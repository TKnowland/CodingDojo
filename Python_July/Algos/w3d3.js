...
function dedupeSorted(nums) {
    let newArr = [];
    for (i in nums) {
        if (!newArr.includes(nums[i])) {newArr += nums[i];}
    }
    return newArr;
}
...
function firstNonRepeated(nums) {
    ...
    for (j in nums) {
        if (!repeatValues.includes(nums[j])) {
            return nums[j];
        }
    }
...
console.log(firstNonRepeated(twoNums5))
...
//     var longest;
//     var interLeave = []
//     arrayA.length > arrayB.length ? longest = arrayA.length : longest = arrayB.length
//     for (var i = 0; i < longest; i++) {
//         if (arrayA[i] != null){
//             interLeave.push(arrayA[i])
//         }
//         if (arrayB[i] != null){
//             interLeave.push(arrayB[i])
//         }
//     }
//     return interLeave
//   }
