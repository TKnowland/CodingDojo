...
class User:
    ...
    def get_one(cls, id):
        ...
        result = connectToMySQL('users').query_db(query, id)
        if not result:
            return False
        return cls(result[0])
    ...
    @classmethod
    def update(cls, data):
        query = "UPDATE users SET fist_name = %(first_name)s, last_name = %(last_name)s, email = %(email)s, updated_at = NOW() WHERE id = %(id)s;"
        return connectToMySQL('users').query_db(query, data)
